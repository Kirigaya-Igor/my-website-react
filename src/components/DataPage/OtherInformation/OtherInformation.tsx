...
export const OtherInformation: React.FC = () => {
    ...
    useEffect(() => {
        appAPI.fetchItems(i18n.language, 'otherInformation', showAlert)
            .then((data) => {
                if(data){
                    setMyData(data)
                }
    ...
    useEffect(() => {
        if(user && newItem.length !== 0){
            ...
            appAPI.addItem(item, i18n.language, 'otherInformation', showAlert)
    ...
    useEffect(() => {
        if(deleteItem !== ''){
            appAPI.removeItem(deleteItem, i18n.language, 'otherInformation', showAlert)
    ...
    return (
        <div className='row customBorder'>
            <h3 className='col-12 text-white text-uppercase d-flex justify-content-center mb-4 mt-3'>{t("otherInformation.title")}</h3>
