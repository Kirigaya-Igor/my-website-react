...
export const Experience: React.FC = () => {
    ...
    useEffect(() => {
        appAPI.fetchItems(i18n.language, 'experience', showAlert)
            .then((data) => {
                if(data){
                    setMyData(data)
                }
    ...
    useEffect(() => {
        if(user && newItem.length !== 0){
            ...
            appAPI.addItem(item, i18n.language, 'experience', showAlert)
    ...
    useEffect(() => {
        if(deleteItem !== ''){
            appAPI.removeItem(deleteItem, i18n.language, 'experience', showAlert)
    ...
    return (
        <div className='row customBorder'>
            <h3 className='col-12 text-white text-uppercase d-flex justify-content-center mb-4 mt-3'>{t("experience.title")}</h3>
